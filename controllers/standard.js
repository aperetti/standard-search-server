...
router.get('/pdf/:standard_id', (req, res) => {
  ...
  models.standard.findById(standardId)
    .then(standard => {
      if (!standard) return Promise.reject()
...
router.get('/lookup/:standard_id', (req, res) => {
  ...
  models.standard.findById(standardId, {
      include: [
        ...
        { model: models.standard, as: 'references', attributes: ['id', 'code', 'description'] },
        { model: models.standard, as: 'referrers', attributes: ['id', 'code', 'description'] }
      ]
...
router.get('/valid_standard/:code', function(req, res) {
  models.standard.findOne({ where: { code: req.params.code } })
...
router.get('/menu/:id', (req, res) => {
  ...
  var includes = [
    { model: models.menu, as: 'parent', include: [{model: models.menu, as: 'children'}] },
...
router.get('/get_setting/', (req, res) => {
  var id = req.query.id
  models.setting.findById(id)
    .then(setting => res.status(200).json(setting))
    .catch(e => {
      console.log(e)
      res.status(400).send('Could not find setting!')
    })
})
